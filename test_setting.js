    ...
    if (window.fbDb) {
        window.fbOnValue(window.getAppRef(getSettingsPath()), (snapshot) => {
            ...
            if (data && data._elements) {
                const _elems = Array.isArray(data._elements) ? data._elements : Object.values(data._elements);
                elements = _elems.filter(e => e).map(e => ({ id: e.id||e.label, label: e.label||e.id, preview_text: e.preview_text||e.label||'', x: parseInt(e.x_pos)||0, y: parseInt(e.y_pos)||0, size: parseInt(e.font_size)||24, align: e.align||'center', font: e.font_family||'Arial', color: e.color||'#000000', bold: e.is_bold?true:false, mapping: e.mapping||'' }));
            } else if (data) {
                elements = [];
